...
def adaptive_population_size(gen, min_size=20, max_size=100):
    """自适应种群大小"""
    # 早期：大种群增加多样性
...
#     compile_cmd = f"sw_64-sunway-linux-gnu-g++ -O3 -static -std=c++11 {' '.join(options)} {TEST_PROGRAM} -o tester"  # 编译命令
#
#     # 执行编译
...
#         result = subprocess.run(['./tester', PROGRAM_INPUT], capture_output=True, text=True)
...
def parallel_fitness_evaluation(population):
    ...
    # 方法2：使用Manager共享结果（备用方法，复杂但灵活）
    # with Manager() as manager:
    #     results = manager.list([None] * len(population))
    #     with Pool(processes=NUM_CORES) as pool:
    #         pool.starmap_async(evaluate_individual_star, [(idx, ind, results) for idx, ind in enumerate(population)])
    #         pool.close()
    #         pool.join()
    #     return list(results)
...
def evaluate_individual(individual):
    """在隔离环境中评估单个个体（防止文件冲突）"""
    # 创建独立工作目录（避免并行编译冲突）
    with tempfile.TemporaryDirectory(prefix="ga_compile_") as temp_dir:
        try:
            # 复制源文件到临时目录
            shutil.copytree(WORK_DIR, temp_dir, dirs_exist_ok=True)
            os.chdir(temp_dir)
            ...
            # 编译命令
            compile_cmd = f"sw_64-sunway-linux-gnu-g++ -O3 -static -std=c++11 {' '.join(options)} {TEST_PROGRAM} -o tester"
            ...
            for _ in range(3):
                try:
                    result = subprocess.run(['./tester', PROGRAM_INPUT], capture_output=True, text=True, timeout=30)
                    if result.returncode != 0:
                        return 0.0
                    for line in result.stdout.splitlines():
                        line = line.strip()
